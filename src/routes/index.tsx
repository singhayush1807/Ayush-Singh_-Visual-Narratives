...
import { personalInfo, videos } from "@/data";
import type { VideoCategory, VideoEntry } from "@/data";
...
function AnimatedSection({
  ...
  return (
    <motion.section
      ref={ref}
      id={id}
      initial={isVisible ? { opacity: 1, y: 0 } : { opacity: 0, y: 32 }}
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50">
      <div className="mx-auto max-w-7xl px-6 py-5 flex items-center justify-between">
        ...
        <Magnetic>
          <a href="mailto:ayushkumar180404@gmail.com" className="pill-btn text-sm hover:bg-primary hover:text-primary-foreground">
...
function Hero() {
  ...
  return (
    ...
    >
      ...
      <motion.div style={{ opacity }} className="relative mx-auto max-w-6xl px-6 text-center">
        <Reveal delay={0.05}>
        ...
        <Reveal delay={0.15}>
          <div className="mt-8 flex flex-wrap items-center justify-center gap-x-4 gap-y-2 font-display text-2xl md:text-3xl text-foreground/80 italic">
        ...
        <Reveal delay={0.25}>
          <p className="mt-8 mx-auto max-w-2xl text-lg leading-relaxed text-foreground/75">
...
function About() {
  ...
  return (
    <AnimatedSection id="about" className="relative py-32 px-6 text-center">
      <div className="mx-auto max-w-6xl grid md:grid-cols-12 gap-12 text-left">
...
function Achievements() {
  const stats = [
    { label: "Average Views / Video", value: 5_000, prefix: "2-", suffix: "K", platform: "YouTube" },
    { label: "Videos Edited", value: 200, suffix: "+", platform: "2023 — Present" },
  ];

  const categories = [
    { icon: Film, label: "YouTube Shorts" },
    { icon: Film, label: "YouTube Long-form" },
    { icon: Zap, label: "Gaming Highlights" },
    { icon: Sparkles, label: "Geopolitics Analysis" },
    { icon: Wand2, label: "Motion Graphics" },
    { icon: Palette, label: "Color Grading" },
    { icon: Music, label: "Sound Design" },
    { icon: Type, label: "Motion Typography" },
    { icon: Zap, label: "Velocity Edits" },
    { icon: Sparkles, label: "Visual Effects" },
    { icon: Film, label: "Instagram Reels" },
    { icon: Wand2, label: "Thumbnail Design" },
  ];
  ...
  return (
    <AnimatedSection className="relative py-32 px-6">
      <div className="mx-auto max-w-6xl text-center">
        ...
        {/* Numeric stats */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 max-w-2xl mx-auto mb-12">
          {stats.map((s, i) => (
        ...
        {/* Category tags */}
        <Reveal delay={0.2}>
          <div className="text-xs uppercase tracking-widest text-primary/60 mb-6">What I specialize in</div>
...
/* ---------------- WORK ---------------- */
type WorkFilter = VideoCategory | "all";
...
*/
const CATEGORIES: { key: WorkFilter; label: string }[] = [
  { key: "gaming-shorts", label: "Gaming Shorts" },
  { key: "geopolitics-shorts", label: "Geopolitics Shorts" },
  { key: "gaming-long-form", label: "Gaming Long-form" },
  { key: "geopolitics-long-form", label: "Geopolitics Long-form" },
  { key: "all", label: "All" },
];

const VIDEO_BATCH_SIZE = 9;

function Work() {
  const [cat, setCat] = useState<WorkFilter>("all");
  ...
  useEffect(() => {
    if (import.meta.env.DEV && filteredVideos.length === 0) {
      console.warn("[Work] No videos matched the active category.", {
        category: cat,
        totalVideos: videos.length,
      });
    }
  }, [cat, filteredVideos.length]);

  const getVideoKind = (video: VideoEntry) =>
    video.category.includes("shorts") ? "shorts" : "longform";
  ...
  return (
    <AnimatedSection id="work" className="relative py-32 px-6">
      <div className="mx-auto max-w-7xl text-center">
        <Reveal>
          <div className="text-xs uppercase tracking-[0.25em] text-primary/70 mb-6">
            <span className="mr-3">03</span> Selected Work
          </div>
          <h2 className="font-display text-6xl md:text-7xl text-primary leading-[0.95] mb-4">
            Recent <em className="italic text-accent">edits</em>
          </h2>
          <p className="mx-auto max-w-md text-foreground/70 mb-12">
            A curated selection across gaming and geopolitics — click any card to play.
          </p>
        </Reveal>
...
function Timeline() {
  ...
  return (
    <AnimatedSection id="journey" className="relative py-32 px-6 bg-cream-deep/30">
      ...
      <div className="mx-auto max-w-5xl paper-card p-8 md:p-14 relative overflow-hidden">
        ...
        <div className="relative text-center">
          <Reveal>
            ...
            <p className="mt-6 mx-auto max-w-md text-foreground/75">
              Have a project, a channel, or an idea you want brought to life? Reach out — I'm always
          ...
          <Reveal delay={0.1}>
            <div className="mt-12 grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-3xl mx-auto">
              {contactLinks.map(({ icon: Icon, label, value, href }) => (
                ...
                >
                  <div className="grid place-items-center h-12 w-12 rounded-full bg-primary text-primary-foreground group-hover:bg-accent transition-colors duration-300">
                    <Icon className="h-5 w-5" />
                  </div>
                  <div className="text-xs uppercase tracking-widest text-primary/60">{label}</div>
                  <div className="font-display text-sm text-primary break-all">{value}</div>
                </a>
              ))}
            </div>
          </Reveal>
        ...
        <footer className="mt-16 pt-8 border-t border-border/40 text-center text-sm text-foreground/50">
